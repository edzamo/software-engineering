# Spring WebFlux — guía de estudio (Parte 2: catálogo de operadores de Reactor)

Continuación de [`webflux.md`](webflux.md), que ya instaló lo básico: por qué existe WebFlux, `Mono` vs `Flux`, y `map` vs `flatMap`. Este documento no es "sintaxis de Reactor" — es "tengo este problema concreto, ¿qué operador lo resuelve y por qué ese y no otro". La frecuencia de uso de cada operador (columna "qué tan común es") viene de auditar microservicios reactivos reales en producción — no es una lista teórica.
...
| `doOnError` | Cuando el flujo termina con una excepción. | Log del error, antes de que `onErrorMap`/`onErrorResume` lo transformen o recuperen (ver [`webflux.md`](webflux.md), sección 5). |
...
| `onErrorResume` / `retryWhen` / `onErrorReturn` | Recuperación de errores (ver [`webflux.md`](webflux.md), sección 5). | **Bajo-medio** en frecuencia absoluta, pero crítico donde aparece (llamadas a servicios externos inestables). |
...
Relacionado: [`webflux.md`](webflux.md) — Parte 1 de esta guía, con `map`/`flatMap`, manejo de errores base, el controlador y el resto del framework — y [`webflux-advanced.md`](webflux-advanced.md) — Parte 3, con `WebFilter`, HTTP/2, streaming/SSE, performance y testing de integración.
