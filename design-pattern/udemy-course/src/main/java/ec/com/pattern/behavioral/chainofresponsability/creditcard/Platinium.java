...
public class Platinium implements ApproveLoanChain {
    ...
    public void crediCardRequest(int totalLoan) {
        if (totalLoan >= 10000 && totalLoan <= 50000) {
            System.out.println("Your loan request for Platinium Card has been approved");
        } else {
            next.crediCardRequest(totalLoan);
            System.out.println("Your loan request for Platinium Card has been declined, please contact with our support");
        }
    }   
}
