...
| [`ia-agentes/`](ia-agentes) | IA aplicada al desarrollo | 🟡 En progreso | 8 agentes **agnósticos de lenguaje** (arquitecto, TDD, developer, revisores, Docker, GitFlow) + skills en 3 categorías (`architecture/`, `quality/`, `stacks/`) + hook que **hace cumplir TDD** + `detect_stack.py`; con `compiler/` traduciendo la fuente a lo que Claude Code auto-descubre. |
...
| [`entrevistas/`](entrevistas) | Bitácora de procesos de entrevista | 🟡 En progreso | [`kaizen/`](entrevistas/kaizen): protocolo de 7 pasos para evaluaciones técnicas en vivo con IA, caso resuelto con rúbrica y retrospectivas. Procesos cerrados en `archivo/`. |
