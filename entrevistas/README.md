...
> La idea es no repetir contenido: lo técnico general (reactivo, microservicios, AWS, arquitectura) vive una sola vez en sus propias carpetas ([`reactive-programming/`](../reactive-programming), [`microservices-patterns/`](../microservices-patterns), etc.). Acá solo queda lo puntual de cada proceso y el mapa de a dónde ir a repasar cada cosa.
